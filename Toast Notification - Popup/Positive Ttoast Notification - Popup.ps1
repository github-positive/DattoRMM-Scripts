...
If($ENV:showRestartOption -eq $true){
    ...
    $closeButton = New-Object System.Windows.Forms.Button
    $closeButton.Text = "Close"
    $closeButton.Location = New-Object System.Drawing.Point([System.Math]::Round(($form.Width - $button.Width) / 2), [System.Math]::Round(($form.Height - $button.Height) - 50)) #button position 60 px from bottom
    $closeButton.Add_Click({ $form.Close() })
}
