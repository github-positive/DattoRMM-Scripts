    ...
    if (($BitlockerStatus -match "Protection\s+Off" -and $BitlockerStatus -match "reboots\s+left") -or ($BitlockerStatus -match "Protection\s+On" -and $BitlockerStatus -match "Percentage Encrypted:\s+100\.0%")) {
        ...
        if ($TPMState.TPMReady -eq $true) {
            ...
            try {
                ...
                Save-CurrentDriveKey -DriveLetter $DriveLetter
...
function Save-CurrentDriveKey {
    ...
    if ($AADJoined) {
        ...
        $BitlockerKeyBackupToAAD = BackupToAAD-BitLockerKeyProtector -MountPoint $DriveLetter -KeyProtectorId $BLV.KeyProtector[1].KeyProtectorId
    ...
    # add a output and only encrypt if key was saved successfully
}
...
if ($ENV:drives -eq 'System') {
    ...
    foreach ($disk in $internalDisks) {
        $DrivesToEncrypt += Get-Partition -DiskNumber $disk | Get-Volume | Where-Object { $_.DriveType -eq 'Fixed' } | Select-Object -ExpandProperty DriveLetter
    }
